...
if (isset($_SESSION['username'])) {
    // Retrieve manager data from the database
    $username = $_SESSION['username'];
    $sql = "SELECT * FROM manager WHERE username = '$username'";
    $resultManager = $conn->query($sql);
    ...
    if ($resultManager && $resultManager->num_rows > 0) {
        ...
        // $managerFullName = $rowManager['full_name'];
        // $managerProfilePhoto = $rowManager['profile_photo'];

        // Fetch all full names from the subscribers table
        $fullNamesQuery = "SELECT full_name FROM subscribers";
        $resultFullNames = $conn->query($fullNamesQuery);

        // Check if there are records
        if ($resultFullNames && $resultFullNames->num_rows > 0) {
            $allFullNames = $resultFullNames->fetch_all(MYSQLI_ASSOC);
        }
    }
  }
                  ...
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" id="subscriberName" read-only name="subscriberName"  class="form-control" placeholder="SLH_000000" required >
                            </div>
                        </div>
